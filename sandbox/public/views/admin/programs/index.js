...
app.HeaderView = Backbone.View.extend({
    ...
    addNew: function() {
        if (this.$el.find('[name="name"]').val() === '') {
            ...
            this.model.save({
                'lecture': this.$el.find('[name="name"]').val()
            },{
...
app.ResultsRowView = Backbone.View.extend({
    ...
    getDuration: function(e) {
        ...
        $.get('/admin/programs/meta' + this.model.attributes._id, function(resp) {
            if (resp.success) {
    ...
    render: function() {
        ...
        var duration = parseInt(this.$('.timeduration').text(), 10);
        if (duration) this.$('.timeduration').text( moment(duration*1000).format('mm:ss') );
